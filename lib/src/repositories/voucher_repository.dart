...
class VoucherRepository extends GetxController {
  ...
  Future<List<VoucherModel>> getAllGroFastVoucher() async {
    try {
      ...
      final groFastVouchers = await _db
          .collection('Vouchers')
  ...
  Future<VoucherModel> getVoucherInformation(String name) async {
    try {
      final documentSnapshot =
          await _db.collection('Vouchers').where('Name', isEqualTo: name).get();
  ...
  Future<VoucherModel> getVoucherInformationFromId(String id) async {
    try {
      final documentSnapshot = await _db.collection('Vouchers').doc(id).get();
